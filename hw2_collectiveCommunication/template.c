...
int main(int argc, char *argv[]) {
  ...
  if (my_rank == ROOT) {
        ...
        A = (double *) malloc(sizeof(double) * elms_to_comm);
        C = (double *) malloc(sizeof(double) * elms_to_comm);
        B = (double *) malloc(sizeof(double) * n_sq);
    }
  ...
  // Scatter rows of matrix A to all processes
    MPI_Scatter(A, elms_to_comm, MPI_DOUBLE, A, elms_to_comm, MPI_DOUBLE, ROOT, MPI_COMM_WORLD);

    // Broadcast matrix B to all processes
    MPI_Bcast(B, n_sq, MPI_DOUBLE, ROOT, MPI_COMM_WORLD);

    // Each process computes its part of matrix multiplication
    mat_mult(A, B, C, n, my_work);
    
    //Barrier helps to complete all the processes.
    MPI_Barrier(world);

    // Gather the result (C) from all processes to the root
    MPI_Gather(C, elms_to_comm, MPI_DOUBLE, C, elms_to_comm, MPI_DOUBLE, ROOT, MPI_COMM_WORLD);
